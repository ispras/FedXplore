...
class FedAvgServer:
    def __init__(self, cfg):
        ...
        self.server_losses = [0 for _ in range(cfg.federated_params.amount_of_clients)]
        self.server_val_df_len = [
            0 for _ in range(cfg.federated_params.amount_of_clients)
        ]
        self.clients_metrics = [
            pd.DataFrame() for _ in range(cfg.federated_params.amount_of_clients)
        ]
        self.clients_losses = [0 for _ in range(cfg.federated_params.amount_of_clients)]
        ...
        self.latest_validation_metrics = None
    ...
    def set_client_result(self, client_result):
        ...
        (
            self.server_metrics[client_result["rank"]],
            self.server_losses[client_result["rank"]],
            self.server_val_df_len[client_result["rank"]],
        ) = client_result["server_metrics"]
        if self.cfg.federated_params.print_client_metrics:
            (
                self.clients_losses[client_result["rank"]],
                self.clients_metrics[client_result["rank"]],
            ) = client_result["client_metrics"]

    def save_best_model(self, round):
        # Collect metrics from clients
        # server_metrics = (metrics, val_loss, len(val_df))
        server_metrics = [
            metrics for metrics in self.server_metrics if len(metrics) != 0
        ]
        val_losses = [loss for loss in self.server_losses if loss != 0]
        val_len_dfs = [df_len for df_len in self.server_val_df_len if df_len != 0]
        ...
        self.latest_validation_metrics = metrics.copy()
        ...
        if rounds_no_improve == 0 and val_loss is not np.nan:
            ...
            self.checkpoint_path = f"{self.model_path}_round_{self.best_round}.pt"
            test_metrics = getattr(self, "last_metrics", None)
