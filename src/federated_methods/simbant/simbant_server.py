...
class SimBANTServer(FLTrustServer):
    def __init__(self, cfg, trust_df, trust_test_samples, prob_temperature):
        ...
        self.trust_test_df = self.trust_df.dataset_split(train_val_prop)
    ...
    def trust_eval_models(self):
        ...
        if targets_tensor.ndim == 1:
            ...
            ohe_targets = F.one_hot(
                targets_tensor.to(torch.long), num_classes=self.trust_df.num_classes
            ).float()
        elif (
            targets_tensor.ndim == 2
            and targets_tensor.shape[1] == self.trust_df.num_classes
        ):
