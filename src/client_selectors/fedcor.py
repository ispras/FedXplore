...
class FedCor(BaseSelector):
    ...
    def change_functionality(self, trainer):
        ...
        trainer.server.gpr = None
        trainer.server.ts = None
        trainer.server.gt_global_losses = []
        trainer.server.warmup = self.warmup
        trainer.warmup = self.warmup
        trainer.amount_of_clients = trainer.server.amount_of_clients
        trainer.server.local_rnd = rand.Random()
        ...
        trainer.server.select_clients_to_train = MethodType(
            FedCor.select_clients_to_train, trainer.server
        )
        ...
        if self.cur_round >= 1:
            if self.cur_round <= self.warmup:  # warm-up
                self.server.gpr.Update_Training_Data(
                    ...
                    [
                        np.array(self.server.gt_global_losses[-1])
                        - np.array(self.server.gt_global_losses[-2]),
                    ],
...
class GPR(torch.nn.Module):
    ...
    def Covariance(self, ids=None):
        raise NotImplementedError(
            "A GPR class must have a function to calculate covariance matrix"
        )
    ...
    def Update_Training_Data(self, client_idxs, loss_changes, epoch):
        ...
        data = np.concatenate(
            [
                np.expand_dims(np.array(client_idxs), 2),
                np.expand_dims(np.array(loss_changes), 2),
            ],
            2,
        )
    ...
    def Posteriori(self, data):
        ...
        mu = (
            self.mu
            + (
                (Cov[:, indices].mm(Sigma_inv)).mm(
                    (values - self.mu[indices]).unsqueeze(1)
                )
            ).squeeze()
        )
    ...
    def Parameter_Groups(self):
        raise NotImplementedError(
            "A GPR class must have a function to get parameter groups = [Mpar,Spar]"
        )
    ...
    def Train(
        ...
        optimizer = torch.optim.Adam(
            [{"params": matrix_params, "lr": lr}, {"params": sigma_params, "lr": llr}],
            lr=lr,
            weight_decay=0.0,
        )
        if schedule_lr:
            lr_scd = torch.optim.lr_scheduler.MultiStepLR(
                optimizer, schedule_t, gamma=schedule_gamma
            )
        ...
        for epoch in range(max_epoches):
            ...
            for e in self.data.keys():
                if self.loss_type == "LOO":
                    loss -= self.Log_LOO_Predictive_Probability(self.data[e]) * (
                        self.gamma ** (current_epoch - e)
                    )
                elif self.loss_type == "MML":
                    loss -= self.Log_Marginal_Likelihood(self.data[e]) * (
                        self.gamma ** (current_epoch - e)
                    )
                else:
            ...
            if epoch % 10 == 0 and verbose:
                print(
                    "Train_Epoch:{}\t|Sigma:{:.4f}\t|Loss:{:.4f}".format(
                        epoch,
                        torch.mean(torch.diagonal(self.Covariance())).detach().item(),
                        loss.item(),
                    )
                )
    ...
    def Predict_Loss(self, data, priori_idx, posteriori_idx):
        ...
        pdist = MultivariateNormal(
            loc=mu_p[posteriori_idx],
            covariance_matrix=sigma_p[posteriori_idx, :][:, posteriori_idx],
        )
        predict_loss = -pdist.log_prob(
            torch.tensor(data[posteriori_idx, 1], device=self.device, dtype=torch.float)
        )
        predict_loss = predict_loss.detach().item()
        return predict_loss, mu_p, sigma_p

    def Select_Clients(
        self, number=10, epsilon=0.0, weights=None, Dynamic=False, Dynamic_TH=0.0
    ):
        """
        ...
        def max_loss_decrease_client(client_group, Sigma, weights=None):
            ...
            Diag_valid = self.discount[client_group] / torch.sqrt(
                torch.diagonal(Sigma_valid)
            )  # alpha_k/sigma_k
            ...
            if weights is None:
                ...
                total_loss_decrease = (
                    torch.sum(
                        torch.tensor(
                            weights[client_group], device=self.device, dtype=torch.float
                        ).reshape([len(client_group), 1])
                        * Sigma_valid,
                        dim=0,
                    )
                    * Diag_valid
            ...
            p_Sigma = Sigma - Sigma[:, selected_idx : selected_idx + 1].mm(
                Sigma[selected_idx : selected_idx + 1, :]
        ...
        for i in range(number):
            idx, Sigma, total_loss_decrease = max_loss_decrease_client(
                remain_clients, Sigma, weights
            )
...
class SE_Kernel(torch.nn.Module):
    ...
    def forward(self, xs):
        ...
        """
        x_size = xs.size(1)
        A = torch.sum(xs**2, dim=0, keepdim=True) * torch.ones(
...
class Kernel_GPR(GPR):
        ...
        **Kernel_Arg
        ...
        class Index_Projection(torch.nn.Module):
            ...
            def __init__(self, num_users, dimension=10):
                ...
                self.PMatrix = Parameter(
                    torch.randn(dimension, num_users) / np.sqrt(dimension)
                )
        ...
        super(Kernel_GPR, self).__init__(
            num_users, loss_type, init_noise, reusable_history_length, gamma, device
        )
    ...
    def Covariance(self, ids=None):
        ...
        return self.Kernel(xs) + (self.noise**2) * torch.eye(
            len(ids), device=self.device
        )

    def Parameter_Groups(self):
        proj_parameters = [
            self.Projection.PMatrix,
        ]
        sigma_parameters = (
            [
                self.Kernel.sigma_f,
            ]
            if hasattr(self.Kernel, "sigma_f")
            else None
        )
...
class Matrix_GPR(GPR):
    ...
    def __init__(
        ...
        super(Matrix_GPR, self).__init__(
            num_users,
            loss_type,
            init_noise,
            reusable_history_length,
            gamma,
            device=device,
        )
        ...
        self.index = torch.zeros(
            (num_users * num_users - num_users) // 2, dtype=torch.long
        )
        n = 0
    ...
    def Covariance(self, ids=None):
        ...
        if ids is None:
            ...
            return Sigma + (self.noise**2) * torch.eye(
                self.num_users, device=self.device
            )
        else:
            return Sigma[ids, :][:, ids] + (self.noise**2) * torch.eye(
                len(ids), device=self.device
            )
