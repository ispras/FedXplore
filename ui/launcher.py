...
def get_mlflow_defaults(repo_root: Path) -> dict[str, str]:
    ...
    tracking_uri = env_tracking_uri or str(config_values.get("tracking_uri", "")).strip()
...
def update_mlflow_url_from_log(run_dir: Path, status: dict[str, Any]) -> dict[str, Any]:
    ...
    mlflow_url = extract_mlflow_run_url_from_text(log_text)
    if not mlflow_url or mlflow_url == status.get("mlflow_url"):
        return status

    status["mlflow_url"] = mlflow_url
    write_status(run_dir, status)
    (run_dir / "mlflow_url.txt").write_text(mlflow_url, encoding="utf-8")

    spec = read_spec(run_dir)
    if spec and spec.get("mlflow_url") != mlflow_url:
        spec["mlflow_url"] = mlflow_url
